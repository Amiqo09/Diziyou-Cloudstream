...
class DiziYou : MainAPI() {
  ...
  override suspend fun load(url: String): LoadResponse? {
    ...
    //<span class="dizimeta"><i class="fas fa-globe"></i> Yapım Yılı : </span>2018<br>
    val poster = fixUrlNull(document.selectFirst("[property='og:image']")?.attr("content"))
    val year = document.selectXpath("//span[text()='Yapım Yılı : ']//following-sibling::text()[1]").text().trim().toIntOrNull()
    val description = document.selectFirst("div.summary p")?.text()?.trim()
    val tags = document.selectXpath("//div[@class='genres']/a").map {
      it.text().trim()
    }
    val rating = document.selectXpath("//span[text()='IMDB : ']//following-sibling::text()[1]").text().trim().toFloatOrNull()
    val duration = Regex("(\\d+)").find(document.selectXpath("//div[text()='Ortalama Süre']//following-sibling::div").text() ?: "")?.value?.toIntOrNull()

    val title = document.selectFirst("//div[@id='gorunum']/span[@class='grtext']")?.text() ?: return null

val episodes = document.select("//div[@id='butonlar']/button").mapNotNull {
    val epName = it.selectFirst("@search-text")?.text()?.trim() ?: return@mapNotNull null
